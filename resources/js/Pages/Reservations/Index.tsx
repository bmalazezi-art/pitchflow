...
const quickFilters = ['today', 'tomorrow', 'week', 'paid', 'unpaid', 'confirmed', 'cancelled', 'pending'] as const;
...
export default function Reservations({ reservations, filters, timezone }: { reservations: Paginated<any>; filters: { search?: string; filter?: string }; timezone: string }) {
    ...
    return <AppLayout title={t('reservations')}><Head title={t('reservations')} /><div className="owner-page">
        <PageHeader eyebrow={t('management')} title={t('reservations')} description={t('reservationsIntro')} actions={<Link className="btn btn-primary" href="/calendar"><CalendarPlus size={18} />{t('newReservation')}</Link>} />
