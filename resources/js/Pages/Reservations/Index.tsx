...
export default function Reservations({ reservations, correctionRequests = [], filters, summary, timezone }: { reservations: Paginated<any>; correctionRequests?: any[]; filters: Filters; summary: Summary; timezone: string }) {
    ...
    const defaultEmployeePermissions = ['create_reservations', 'edit_reservations', 'cancel_reservations', 'view_customers', 'add_customer_notes', 'view_calendar', 'view_assigned_fields'];
    const permissions = auth.user?.permissions ?? defaultEmployeePermissions;
    const canCreateReservations = auth.user?.role === 'employee' && permissions.includes('create_reservations');
    const canEditReservations = auth.user?.role === 'employee' && permissions.includes('edit_reservations');
    const canCancelReservations = auth.user?.role === 'employee' && permissions.includes('cancel_reservations');
    ...
    return <AppLayout title={t('reservations')}><Head title={t('reservations')} /><div className="owner-page">
        <PageHeader eyebrow={t('management')} title={t('reservations')} description={t('reservationsIntro')} actions={canCreateReservations ? <Link className="btn btn-primary" href="/calendar"><CalendarPlus size={18} />{t('newReservation')}</Link> : <span className="read-only-indicator">{t('readOnly')}</span>} />
