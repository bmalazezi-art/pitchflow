...
export default function Calendar({ reservations, fields, selectedField }: { reservations: any[]; fields: any[]; timezone: string; selectedField?: number | null }) {
    ...
    const form = useForm({ customer_name: '', customer_phone: '', football_field_id: selectedField ?? fields[0]?.id ?? '', starts_at: '', ends_at: '', status: 'confirmed', payment_status: 'unpaid', paid_amount: 0, is_walk_in: false, notes: '' });
    ...
    return <AppLayout title={t('calendar')}><Head title={t('calendar')} /><div className="owner-page calendar-page">
        <PageHeader eyebrow={t('schedule')} title={t('calendar')} description={t('calendarHelp')} actions={<Button onClick={() => showNew()}><CalendarPlus size={18} />{t('newReservation')}</Button>} />
        <section className="calendar-shell">
            ...
            <FullCalendar plugins={[dayGridPlugin, timeGridPlugin, interactionPlugin]} locales={[sqLocale]} locale={locale} initialView={window.innerWidth < 768 ? 'timeGridDay' : 'timeGridWeek'} headerToolbar={{ left: 'prev,next today', center: 'title', right: 'dayGridMonth,timeGridWeek,timeGridDay' }} buttonText={{ today: t('today'), month: t('month'), week: t('week'), day: t('day') }} slotMinTime="12:00:00" slotMaxTime="26:00:00" slotDuration="01:00:00" slotLabelInterval="01:00:00" allDaySlot={false} height="auto" nowIndicator editable selectable dateClick={(arg: DateClickArg) => showNew(arg.date)} eventDrop={drop} events={events} eventClick={info => showEdit(info.event.extendedProps.reservation)} eventDidMount={info => { info.el.title = `${info.event.title}\n${info.event.extendedProps.reservation.customer_phone}`; }} />
        </section>
        <Modal open={open} title={editing ? t('editReservation') : t('newReservation')} onClose={() => setOpen(false)}><form onSubmit={submit}><div className="form-grid">
            ...
            <Field label={t('selectField')} error={form.errors.football_field_id} required><Select value={form.data.football_field_id} onChange={event => form.setData('football_field_id', Number(event.target.value))}>{fields.map(field => <option key={field.id} value={field.id}>{field.name}</option>)}</Select></Field><Field label={t('status')}><Select value={form.data.status} onChange={event => form.setData('status', event.target.value)}><option value="pending">{t('pending')}</option><option value="confirmed">{t('confirmed')}</option><option value="completed">{t('completed')}</option><option value="no_show">{t('noShow')}</option></Select></Field>
            ...
            <Field label={t('payment')} error={form.errors.payment_status}><Select value={form.data.payment_status} onChange={event => form.setData('payment_status', event.target.value)}><option value="unpaid">{t('unpaid')}</option><option value="partial">{t('partial')}</option><option value="paid">{t('paid')}</option></Select></Field><Field label={t('amountPaid')}><Input type="number" min="0" step=".01" value={form.data.paid_amount} onChange={event => form.setData('paid_amount', Number(event.target.value))} /></Field>
