...
export default function Register({ cities }: { cities: Array<{ id: number; name: string }> }) {
    ...
    return <AuthLayout><Head title={t('register')} /><h1>{t('register')}</h1><p>{t('applicationsReviewed')}</p>
        <form onSubmit={(e) => { e.preventDefault(); form.post('/register'); }}>
            <div className="form-grid">
                <Field label={t('fullName')} error={form.errors.name} required><Input value={form.data.name} onChange={(e) => form.setData('name', e.target.value)} /></Field>
                ...
                <Field label={t('businessName')} error={form.errors.business_name} required><Input value={form.data.business_name} onChange={(e) => form.setData('business_name', e.target.value)} /></Field>
                ...
                <Field label={t('selectCity')} error={form.errors.city_id} required><Select value={form.data.city_id} onChange={(e) => form.setData('city_id', e.target.value)}><option value="">{t('select')}</option>{cities.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</Select></Field>
                <Field label={t('numberOfFields')} error={form.errors.number_of_fields} required><Input type="number" min={1} value={form.data.number_of_fields} onChange={(e) => form.setData('number_of_fields', Number(e.target.value))} /></Field>
                ...
                <Field label={t('confirmPassword')} error={form.errors.password_confirmation} required><Input type="password" value={form.data.password_confirmation} onChange={(e) => form.setData('password_confirmation', e.target.value)} /></Field>
            </div>
