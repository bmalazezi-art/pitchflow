...
export default function Login() {
    ...
    return <AuthLayout><Head title={t('login')} /><h1>{t('login')}</h1><p>{t('accessWorkspace')}</p>
        <form onSubmit={(e) => { e.preventDefault(); form.post('/login'); }}>
            ...
            <div><Link href="/forgot-password">{t('forgotPassword')}</Link> · <Link href="/register">{t('register')}</Link></div>
        </form>
    </AuthLayout>;
}
