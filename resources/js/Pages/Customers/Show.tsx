...
export default function CustomerShow({ customer, fields }: { customer: any; fields: Array<{ id: number; name: string }> }) {
    ...
    const canManageCustomer = auth.user?.role === 'owner' || auth.user?.role === 'employee';
    ...
    return <AppLayout title={customer.name}>
        ...
        <section className="metrics-grid">
            <div className="metric"><span>{t('reliability')}</span><strong>{reliabilityLabel(customer.reliability_status)}</strong></div>
            <div className="metric"><span>{t('totalBookings')}</span><strong>{customer.total_reservations}</strong></div>
            <div className="metric"><span>{t('lastBooking')}</span><strong>{customer.last_visit_at ? new Date(customer.last_visit_at).toLocaleDateString() : t('noData')}</strong></div>
            ...
            <div className="metric"><span>{t('unpaidBookings')}</span><strong>{customer.unpaid_reservations_count ?? 0}</strong></div>
        </section>
            ...
            <div style={{ display: 'grid', gap: 18 }}>
                <section className="panel">
                    ...
                    {canManageCustomer ? <form onSubmit={event => { event.preventDefault(); profile.put(`/customers/${customer.id}`); }} style={{ display: 'grid', gap: 12 }}>
                        ...
                        <Field label={t('reliability')} error={profile.errors.reliability_status}><Select value={profile.data.reliability_status} onChange={event => profile.setData('reliability_status', event.target.value)}><option value="reliable">{t('reliabilityGood')}</option><option value="needs_attention">{t('reliabilityWatch')}</option><option value="high_risk">{t('reliabilityBlocked')}</option></Select></Field>
