...
export default function Customers({ customers, filters }: { customers: Paginated<any>; filters: { search?: string } }) {
    ...
    return <AppLayout title={t('customers')}><Head title={t('customers')} /><div className="page-header"><div><h1>{t('customers')}</h1><p>Private, organization-specific customer history.</p></div></div>
        ...
        {customers.data.length === 0 ? <div className="panel"><EmptyState title={t('noResults')} /></div> : <><div className="table-wrap"><table><thead><tr><th>{t('name')}</th><th>{t('status')}</th><th>Total</th><th>Completed</th><th>No-shows</th><th>Late cancellations</th><th /></tr></thead><tbody>{customers.data.map(c => <tr key={c.id}><td><strong>{c.name}</strong><br /><small>{c.phone}</small></td><td><Badge value={c.reliability_status} /></td><td>{c.total_reservations}</td><td>{c.completed_reservations}</td><td>{c.no_shows}</td><td>{c.late_cancellations}</td><td><Link className="icon-btn" href={`/customers/${c.id}`}><ChevronRight size={18} /></Link></td></tr>)}</tbody></table></div><Pagination links={customers.links} /></>}
    </AppLayout>;
}
