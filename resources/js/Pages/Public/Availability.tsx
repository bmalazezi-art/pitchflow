...
export default function Availability({ cities, fields, selectedField, slots, filters }: Props) {
    ...
    return <div className="public-page"><Head title={t('checkAvailability')} />
        <nav className="public-nav"><div className="brand"><span className="brand-mark">P</span><strong>PitchFlow</strong></div><div><Link href="/login" className="btn btn-secondary">{t('login')}</Link> <Link href="/register" className="btn btn-primary"><span className="desktop-label">{t('register')}</span><span className="mobile-label">{t('registerShort')}</span></Link></div></nav>
        <main className="public-content"><CalendarDays size={34} color="#2563eb" /><h1>{t('checkAvailability')}</h1><p>{t('publicIntro')}</p>
            ...
            {selectedField && <div className="page-header" style={{ marginTop: 28 }}><div><h2>{selectedField.organization.name} · {selectedField.name}</h2><p>{t('privacyNotice')}</p></div></div>}
