...
export default function Employees({ employees, fields, stats }: { employees: Paginated<any>; fields: Array<{ id: number; name: string }>; stats: { total: number; active: number; invited: number; disabled: number } }) {
    ...
    const submit = () => {
        ...
        editing ? form.put(`/employees/${editing.id}`, { onSuccess: () => setOpen(false) }) : form.post('/employees', { onSuccess: () => setOpen(false) });
    };
    ...
    const resendInvitation = (employee: any, copy = false) => {
        ...
        router.post(`/employees/${employee.id}/resend-invitation`, {}, {
            onFinish: () => setEmployeeActionId(null),
    ...
    const createResetLink = (employee: any, copy = false) => {
        ...
        setCopied(false);
        setEmployeeActionId(employee.id);
        router.post(`/employees/${employee.id}/reset-password-link`, {}, {
            onFinish: () => setEmployeeActionId(null),
            onSuccess: () => setOpen(false),
        });
    };
    return <AppLayout title={t('employees')}><Head title={t('employees')} /><div className="owner-page">
        ...
        {employees.data.length === 0 ? <section className="dashboard-panel"><EmptyState title={t('noEmployees')} action={<Button onClick={() => show()}><MailPlus size={17} />{t('inviteEmployee')}</Button>} /></section> : <>
            <div className="table-wrap modern-table employee-table"><table><thead><tr><th>{t('employee')}</th><th>{t('phone')}</th><th>{t('assignedFields')}</th><th>{t('status')}</th><th>{t('lastLogin')}</th><th>{t('actions')}</th></tr></thead><tbody>{employees.data.map(employee => <tr key={employee.id} onClick={() => show(employee)} className="clickable-row">
                <td data-label={t('employee')}><div className="identity-cell"><span><UserRound size={17} /></span><div><strong>{employee.name}</strong><small>{employee.email || employee.phone}</small></div></div></td><td data-label={t('phone')}>{employee.phone || t('none')}</td><td data-label={t('assignedFields')}><div className="tag-list">{employee.assigned_fields.length ? employee.assigned_fields.map((field: any) => <span key={field.id}>{field.name}</span>) : t('none')}</div></td><td data-label={t('status')}><Badge value={employee.status ?? 'active'} /></td><td data-label={t('lastLogin')}>{employee.last_login_at ? new Intl.DateTimeFormat(undefined, { dateStyle: 'short', timeStyle: 'short', hourCycle: 'h23' }).format(new Date(employee.last_login_at)) : t('never')}</td><td data-label={t('actions')}><div className="row-action-group"><button className="icon-btn bordered" onClick={event => { event.stopPropagation(); show(employee); }} title={t('edit')}><Pencil size={17} /></button>{['active', 'invited'].includes(employee.status) && <button className="icon-btn bordered" disabled={employeeActionId === employee.id} onClick={event => { event.stopPropagation(); createResetLink(employee, true); }} title={t('resetPassword')} aria-label={t('resetPassword')}><KeyRound size={17} /></button>}</div></td>
            </tr>)}</tbody></table></div>{employees.last_page > 1 && <Pagination links={employees.links} />}</>}
        <Drawer open={open} title={editing ? editing.name : t('inviteEmployee')} subtitle={editing?.email ?? editing?.phone ?? t('secureInvitationHelp')} onClose={() => setOpen(false)} footer={<><Button variant="secondary" disabled={form.processing || Boolean(employeeActionId)} onClick={() => setOpen(false)}>{t('cancel')}</Button>{editing?.status === 'invited' && <><Button variant="secondary" disabled={employeeActionId === editing.id} onClick={() => resendInvitation(editing)}><RefreshCw size={16} />{employeeActionId === editing.id ? t('processing') : t('resendInvitation')}</Button><Button variant="secondary" disabled={employeeActionId === editing.id} onClick={() => resendInvitation(editing, true)}><Copy size={16} />{employeeActionId === editing.id ? t('processing') : t('copyInviteLink')}</Button></>}{editing && ['active', 'invited'].includes(editing.status) && <Button variant="secondary" disabled={employeeActionId === editing.id} onClick={() => createResetLink(editing, true)}><KeyRound size={16} />{employeeActionId === editing.id ? t('processing') : t('resetPassword')}</Button>}{editing && <Button variant="secondary" disabled={Boolean(employeeActionId)} onClick={() => router.patch(`/employees/${editing.id}/status`, {}, { onSuccess: () => setOpen(false) })}>{editing.status === 'disabled' ? <CheckCircle2 size={16} /> : <Ban size={16} />}{editing.status === 'disabled' ? t('enable') : t('disable')}</Button>}<Button disabled={form.processing || Boolean(employeeActionId)} onClick={submit}>{form.processing ? t('saving') : editing ? t('saveChanges') : t('sendInvitation')}</Button></>}>
