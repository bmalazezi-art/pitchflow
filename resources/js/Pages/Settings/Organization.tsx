...
export default function OrganizationSettings({ organization, cities }: { organization: any; cities: Array<{ id: number; name: string }> }) {
    ...
    return <AppLayout title={t('settings')}><Head title={t('settings')} /><div className="page-header"><div><h1>{t('organizationSettings')}</h1><p>{t('settingsIntro')}</p></div></div>
        <section className="panel" style={{ maxWidth: 820 }}><form onSubmit={e => { e.preventDefault(); form.put('/settings/organization'); }}><div className="form-grid">
            <Field label={t('businessName')} error={form.errors.name}><Input value={form.data.name} onChange={e => form.setData('name', e.target.value)} /></Field><Field label={t('businessEmail')} error={form.errors.email}><Input type="email" value={form.data.email} onChange={e => form.setData('email', e.target.value)} /></Field>
            ...
            <Field label={t('address')} error={form.errors.address}><Input value={form.data.address} onChange={e => form.setData('address', e.target.value)} /></Field><Field label={t('timezone')} error={form.errors.timezone}><Input value={form.data.timezone} onChange={e => form.setData('timezone', e.target.value)} /></Field>
            <Field label={t('currency')} error={form.errors.currency}><Input maxLength={3} value={form.data.currency} onChange={e => form.setData('currency', e.target.value.toUpperCase())} /></Field><Field label={t('language')}><Select value={form.data.locale} onChange={e => form.setData('locale', e.target.value)}><option value="en">English</option><option value="sq">Shqip</option></Select></Field>
            <Field label={t('cancellationWindow')} error={form.errors.cancellation_window_minutes}><Input type="number" min="0" value={form.data.cancellation_window_minutes} onChange={e => form.setData('cancellation_window_minutes', Number(e.target.value))} /></Field>
