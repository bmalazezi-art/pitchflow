...
class BusinessSuspended extends Notification
{
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        $message = (new MailMessage)
            ->subject(__('messages.business_suspended_subject', [], $locale))
            ->greeting(__('messages.business_email_greeting', ['name' => $notifiable->name], $locale))
            ->line(__('messages.business_suspended_line', [], $locale))
            ->line(__('messages.business_email_business_label', [], $locale))
            ->line($this->organization->name);

        if (filled($this->reason)) {
            $message->line(__('messages.business_suspended_reason', ['reason' => $this->reason], $locale));
        }
